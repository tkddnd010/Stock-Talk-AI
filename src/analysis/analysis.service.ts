...
export class AnalysisService {
    ...
    async runMultiAgentDebate(symbol: string, currentPrice: number, changePercent: number){
        ...
        const collectedAnalyses = await this.publishIndividualAnalyses(symbol, currentPrice, changePercent);

        await this.conductComprehensiveDebate(symbol, currentPrice, changePercent, collectedAnalyses);
    }

    private async publishIndividualAnalyses(symbol: string, currentPrice: number, changePercent: number){
        ...
        for (const persona of allPersona){
            try{
                ...
                const result = await this.aiService.getAnalysis(prompt, symbol, changePercent, currentPrice);
                ...
                this.logger.error(`${persona} 분석글 저장 중 오류 발생: ${error.message}`);
            }
        }
        return collected;
    }

    private async conductComprehensiveDebate(symbol: string, currentPrice: number, changePercent: number, analyses:any){
