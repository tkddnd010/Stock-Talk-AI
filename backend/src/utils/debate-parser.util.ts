...
export function parseDebateScript(
    ...
    for (let line of lines) {
        ...
        if (speakerMatch && !line.match(/^[0-9]+:/)) {
            // "1:30" 같은 시간 표시나 번호 매기기 방어
            ...
            else if (currentSpeaker.includes('기술적분석가'))
                currentRole = 'TECH';
            else if (
                currentSpeaker.includes('사회자') ||
                currentSpeaker.includes('중재자')
            )
                currentRole = 'MODERATOR';
            else currentRole = 'PANEL';
        } else if (
            bracketMatch &&
            (bracketMatch[1].includes('분석가') ||
                bracketMatch[1].includes('투자자'))
        ) {
            flushBlock();
