...
export class AnalysisService {
    ...
    private async conductComprehensiveDebate(
        ...
        [필수 출력 규칙]
        1. 발언자의 이름표는 절대로 임의로 만들지 마세요. (TA, 모더레이터, 가치투자자(반박) 등 사용 금지)
        2. 반드시 지정된 이름표 딱 3가지만 사용하세요: [사회자], [가치투자자], [기술적분석가]
        3. 각 발언이 끝날 때마다 반드시 줄바꿈을 해서 다음 발언자와 명확히 분리되게 작성하세요.
        4. 토론의 마지막 요약 및 권고는 반드시 [사회자] 이름표를 달고 출력하세요.

        [출력 포맷 예시 - 이 양식을 무조건 따르세요]
        [사회자] 토론을 시작하겠습니다. 주가가...
